...
public:
	// 选项有四个
	// 1. linear_search_window & angular_search_window 是传给 search para 的
	// 2. linear weight & angular weight 是用来计算匹配结果的
	...
	// returns the score.
	// 前3个为匹配需要的信息，最后1个参数保存计算出的对当前位姿的估计
	// 返回的是匹配的得分
	...
	//
	// Visible for testing.
	// 这个函数本来可以定义为私有函数，为了测试才放在 public 下
