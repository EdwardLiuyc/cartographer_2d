...
public:
  ...
  FastCorrelativeScanMatcher& operator=(const FastCorrelativeScanMatcher&) =
	...
	// with the result.
	/* 	
		实际调用的是函数 MatchWithSearchParameters
		这里和　RealTimeCorrelativeScanMatcher 有一些区别
		Real在构造时不需要 probability_grid，所以在 match 函数调用时需要传入 grid
		Fast在构造时传入 grid，所以 match 只需要传入 point_cloud
		返回值为是否 match 成功	
	*/
