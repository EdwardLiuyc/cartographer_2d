		...
		transform::Rigid2d* pose_estimate) const 
{
	...
	// step1 用传入参数计算，完成匹配的准备工作，包括旋转、离散化等
	...
	// 生成了一个 candidate 的vector，主要是针对 search 的平移
	std::vector<Candidate> candidates = GenerateExhaustiveSearchCandidates(search_parameters);
	
	// 计算整个 candidate vector 中所有候选的得分
	// 这个函数是 match 的核心
	ScoreCandidates(probability_grid, discrete_scans, search_parameters,&candidates);

	// 只比 score，找到得分最高的 candidate
    ...
    std::vector<Candidate>* const candidates) const 
{
	for (Candidate& candidate : *candidates) 
	{
		...
		candidate.score /= static_cast<float>(discrete_scans[candidate.scan_index].size());
		
		// 计算的最后加入两个权重来得到最终的 score
		// 这个系数始终小于 1，计算式为
		// e^( -(t_weight *(x+y) + r_weight*r)^2 )
		candidate.score *=
			std::exp(
				-common::Pow2(
						std::hypot(candidate.x, candidate.y) * options_.translation_delta_cost_weight() +
