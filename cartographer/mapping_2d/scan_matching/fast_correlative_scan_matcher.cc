    ...
    transform::Rigid2d* pose_estimate) const 
{
	...
	// fully.
	// 在 FCSM 的 MatchWithSearchParameters 会对linear search范围做进一步的调整
	// 所以在这里直接给 1e6 × resolution 只是一个余量很大的初值，并不一定这么大
...
FastCorrelativeScanMatcher::GenerateLowestResolutionCandidates(
    const SearchParameters& search_parameters) const 
{
	// 先获取一个合理的xy分辨率，再用与 real time 算法中类似的方法生成一系列的 candidate
