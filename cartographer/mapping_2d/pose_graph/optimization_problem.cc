...
transform::Rigid3d OptimizationProblem::ComputeRelativePose(
	...
	const NodeData& first_node_data,
    const NodeData& second_node_data) const 
{
	if (odometry_data_.HasTrajectory(trajectory_id)) 
	{
		...
		const std::unique_ptr<transform::Rigid3d> second_node_odometry = InterpolateOdometry(trajectory_id, second_node_data.time);
	...
	return transform::Embed3D( first_node_data.initial_pose.inverse() * second_node_data.initial_pose );
}
