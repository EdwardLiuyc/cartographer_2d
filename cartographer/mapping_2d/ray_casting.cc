              ...
              ProbabilityGrid* const probability_grid) 
{
	...
	// 这里先将单边的分辨率提高了 1000 倍（2d的情况下整体栅格数提高了 1000000 倍）
	...
	for (const Eigen::Vector3f& hit : range_data.returns) 
	{
		...
		// 在插入到栅格图里面时将分辨率还原
		// 这里用到了在 Range data inserter中初始化的概率表格
		probability_grid->ApplyLookupTable( ends.back() / kSubpixelScale, hit_table );
	}

	// 这个配置如果为 false，则不管空的没扫到地方，只计算认为有障碍物的地方
	// 可能会降低一些计算量，可以尝试修改这个参数 TODO(edward)
