...
// 这个类用于维护和调度多个传感器数据的队列
// 一旦要获取传感器数据，必须取到所有队列中的一个数据才算成功，否则会等待其中的某个或某几个队列被塞入数据
// 比如如果 use_imu_data 被设为 true，则在lidar数据来了之后，要一直等待imu数据
...
public:
	...
	// dispatch data.
	// 可以获取当前堵塞的队列 key,包含了 trajectory_id 和 sensor_id
