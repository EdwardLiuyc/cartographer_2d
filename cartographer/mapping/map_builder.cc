...
{
	// 目前我们主要是2d slam，所以这里这个判断其实可以取消，目前保留对性能没有影响
	if (options.use_trajectory_builder_2d()) 
	{
		pose_graph_2d_ = common::make_unique<mapping_2d::PoseGraph>(options_.pose_graph_options(), &thread_pool_);
...
// GlobalTrajectoryBuilder 是一个抽象类，需要提供 TrajectoryBuilder, TrajectoryBuilderOptions和posegraph 的类型
using GlobalTrajectoryBuilder2d = mapping::GlobalTrajectoryBuilder<
...
TrajectoryBuilder* MapBuilder::GetTrajectoryBuilder(const int trajectory_id) const 
{
