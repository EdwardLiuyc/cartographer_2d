...
ImuTracker::ImuTracker(const double imu_gravity_time_constant,
    ...
    : imu_gravity_time_constant_(imu_gravity_time_constant),
      ...
      gravity_vector_(Eigen::Vector3d::UnitZ()),		/* 这里将重力方向直接初始化为竖直朝上而并非朝下！！ */
      imu_angular_velocity_(Eigen::Vector3d::Zero()),
      track_mode_(kAddAccAndAngularVelocity)
	  {}
...
{
	if( track_mode_ == kAddOrientationDirectly )
		return;
...
void ImuTracker::AddImuOrientationDirectly(  common::Time time, 
											const Eigen::Quaterniond& rotation )
{
	(void)time;
	(void)rotation;
