...
// 这个类并没有实现算法，只是实现了一些配置参数的接口，另外定义了一些论文中一些概念对应的结构体
// 同样的，在pose_graph文件夹里面的类也只是实现了配置接口，真正的算法实现在 mapping_2d 中
class PoseGraph {
 public:
	...
	// 论文里提到，这个算法的最大优势在于它可以不断的更新对所有 node 的全局估计，且计算开销很小
	// 不过论文里的例子使用到了 odometry 为算法提供初始值
	struct Constraint 
	{
		struct Pose 
		{
	...
	// 可以给一个初始的位置，有利于迅速找到在机器人当前在地图（目标trajectory）中的位置
