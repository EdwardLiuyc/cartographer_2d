...
public:
							...
							const sensor::TimedPointCloud& ranges) override 
	{
		std::unique_ptr<typename LocalTrajectoryBuilder::MatchingResult> matching_result 
			= local_trajectory_builder_.AddRangeData(time, sensor::TimedRangeData{origin, ranges, {}});
...
private:
	...
	// PoseGraph 和 LocalTrajectoryBuilder 分别是整个系统的后端和前端
	// 前者做 loop closure， 后者做 scan match
